---
title: Data Browser
description: Browse, query, insert, edit, and delete database records from Northship.
---
...
Northship hides TimescaleDB internal schemas from table lists.
...
Row queries are paginated. Northship caps page size to keep browser actions bounded.
...
ClickHouse browsing is read-only in Northship's current UI.
...
For MySQL, Northship parses tabular output from the `mysql` client.

For ClickHouse, read queries return JSON rows. If the query does not include a `FORMAT`, Northship adds `FORMAT JSONEachRow`.
