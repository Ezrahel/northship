...
Northship databases are services. They live inside projects, have deployments, variables, logs, data tools, settings, domains where applicable, and backups.

## Engines

Northship can create:
...
When you create a database service, Northship generates engine-specific variables such as `POSTGRES_DB`, `POSTGRES_USER`, `POSTGRES_PASSWORD`, `REDIS_PASSWORD`, or `MONGO_INITDB_ROOT_PASSWORD`.
...
When a root domain is configured, Northship can create public database hostnames. The hostname and host port are shown in the database service settings.
