...
description: Finish the first-run setup after Northship is installed on your server.
sidebar:
  order: 2
---
...
You can serve Northship itself through Caddy by choosing a dashboard domain such as:
...
Northship keeps the raw `http://IP:4310` URL available as a fallback.
...
If you do not want to use a GitHub App, Northship can also read a `GITHUB_ACCESS_TOKEN` from the server environment. The GitHub App path is better for repository selection and push webhooks.

## Add the Wildcard Root Domain

Set a wildcard root domain when you want Northship to generate service hostnames automatically:
...
If you have a Cloudflare R2 bucket or token ready, connect R2 in the same step. Northship can then save database backups to disk, R2, or both.
...
## Import an Northship Bundle

If you are moving from another Northship server, use the migration import option during onboarding. Choose the `.northship` file and enter the passphrase from the source server.

The import restores projects, services, users, domains, environment variables, static output, backup records, backup files, database dumps, Caddy config, and system settings. Northship clears auth sessions after import, then sends you through the success path so the restored owner account can sign in cleanly.
