...
System Maintenance is the server health view inside Northship. It helps you catch disk pressure before deployments, backups, or database volumes run the server out of space.

## What Northship Measures

Northship records:

- Root filesystem usage from `df`.
- Docker storage and reclaimable data from Docker system data.
- Northship data directory usage.
...
Northship raises maintenance alerts when:
...
- Northship build artifacts older than 24 hours.
...
- Before exporting an Northship migration bundle.
