...
description: Check, review, and apply Northship updates for git and image installs.
---
...
For installs running from a git checkout, Northship checks the current commit against the remote branch.
...
- `unknown`: Northship could not check status.
...
6. Queue a restart when `NORTHSHIP_UPDATE_RESTART_CMD` is configured.

If the checkout has local changes, Northship refuses to update automatically.

## Image Installs

For Docker image installs, Northship compares the image commit metadata with GitHub. Images need `NORTHSHIP_COMMIT_SHA` to make that comparison useful.
...
```bash
cd /opt/northship && sudo docker compose pull northship && sudo docker compose up -d northship
```
