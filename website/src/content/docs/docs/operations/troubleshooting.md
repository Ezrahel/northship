...
Deployment logs show Northship's job-level view. Runtime logs show container output. Server logs show the underlying control plane, Caddy, and BuildKit behavior.

## BuildKit Is Unavailable

Northship source builds need BuildKit at the configured `BUILDKIT_HOST`, usually `tcp://127.0.0.1:1234`.
...
- The target Northship database is not deployed.
