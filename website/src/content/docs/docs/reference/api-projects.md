...
Project endpoints manage Northship project groups. Services live inside projects.
...
```bash
curl -X POST "$NORTHSHIP_URL/api/projects" \
  -H "Authorization: Bearer $NORTHSHIP_API_KEY" \
...
```bash
curl "$NORTHSHIP_URL/api/projects/acme" \
  -H "Authorization: Bearer $NORTHSHIP_API_KEY"
```
...
```bash
curl -X PATCH "$NORTHSHIP_URL/api/projects/project_123" \
  -H "Authorization: Bearer $NORTHSHIP_API_KEY" \
  -H "Content-Type: application/json" \
...
```bash
curl -X DELETE "$NORTHSHIP_URL/api/projects/project_123" \
  -H "Authorization: Bearer $NORTHSHIP_API_KEY"
```
...
```bash
curl "$NORTHSHIP_URL/api/projects/project_123/database-variable-suggestions" \
  -H "Authorization: Bearer $NORTHSHIP_API_KEY"
```
