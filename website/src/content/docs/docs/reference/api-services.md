...
`dockerfilePath` optionally points at a Dockerfile in a non-standard location, relative to the service root directory (for example `docker/Dockerfile.web`). The `NORTHSHIP_DOCKERFILE_PATH` service environment variable takes precedence when set. Custom install, build, and start commands do not apply to Dockerfile builds.
...
```bash
curl "$NORTHSHIP_URL/api/services/svc_web/overview" \
  -H "Authorization: Bearer $NORTHSHIP_API_KEY"
```
...
```bash
curl -X DELETE "$NORTHSHIP_URL/api/services/svc_web" \
  -H "Authorization: Bearer $NORTHSHIP_API_KEY"
```
