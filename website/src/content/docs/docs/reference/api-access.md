---
title: API Access
description: Authenticate scripts and CI jobs with scoped Northship API keys.
---

API keys let scripts, CI jobs, and local tools call Northship without a browser session.
...
After creation, Northship shows the full key once. Copy it before closing the dialog. Later, the API Access page only shows metadata such as prefix, scope, expiration, and last-used time.
...
```bash
export NORTHSHIP_URL="https://pilot.example.com"
export NORTHSHIP_API_KEY="ap_..."

curl "$NORTHSHIP_URL/api/projects" \
  -H "Authorization: Bearer $NORTHSHIP_API_KEY"
```

Northship also accepts `X-API-Key`:

```bash
curl "$NORTHSHIP_URL/api/projects" \
  -H "X-API-Key: $NORTHSHIP_API_KEY"
```
...
`Specific projects` keys can only access the selected projects. For service routes, Northship resolves the service to its project before allowing the request. For deployment routes, Northship resolves the deployment to its service, then to the service project.
