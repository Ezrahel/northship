...
```bash
curl -X POST "$NORTHSHIP_URL/api/services/svc_web/env" \
...
```bash
curl -X DELETE "$NORTHSHIP_URL/api/services/svc_web/env/env_123" \
  -H "Authorization: Bearer $NORTHSHIP_API_KEY"
```
...
```bash
curl -X POST "$NORTHSHIP_URL/api/services/svc_web/domains" \
  -H "Authorization: Bearer $NORTHSHIP_API_KEY" \
  -H "Content-Type: application/json" \
...
```bash
curl -X DELETE "$NORTHSHIP_URL/api/services/svc_web/domains/domain_123" \
  -H "Authorization: Bearer $NORTHSHIP_API_KEY"
```
...
```bash
curl -X POST "$NORTHSHIP_URL/api/services/svc_web/domains/domain_123/dns-records" \
  -H "Authorization: Bearer $NORTHSHIP_API_KEY" \
  -H "Content-Type: application/json" \
