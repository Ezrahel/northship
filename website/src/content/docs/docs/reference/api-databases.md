...
Database services are Northship services with `repoFullName: "database:<engine>"`.
...
```json
{
  ...
  "env": [
    { "key": "POSTGRES_DB", "value": "northship" },
...
```bash
curl -X POST "$NORTHSHIP_URL/api/projects/project_123/services" \
...
`databasePublicHostname` is optional. When omitted and a root domain is configured, Northship generates the public database hostname from the service name.
...
```json
{
  ...
  "databasePublicEnabled": true,
  "env": [
    { "key": "MYSQL_DATABASE", "value": "northship" },
...
```json
{
  ...
  "databasePublicEnabled": true,
  "env": [
    { "key": "CLICKHOUSE_DB", "value": "northship" },
...
```bash
curl "$NORTHSHIP_URL/api/services/svc_postgres/database/tables" \
  -H "Authorization: Bearer $NORTHSHIP_API_KEY"
```
...
```bash
curl "$NORTHSHIP_URL/api/services/svc_postgres/database/rows?table=public.users&limit=50&offset=0" \
  -H "Authorization: Bearer $NORTHSHIP_API_KEY"
```
...
```bash
curl -L "$NORTHSHIP_URL/api/services/svc_postgres/database/backups/backup_123/download" \
  -H "Authorization: Bearer $NORTHSHIP_API_KEY" \
  -o backup.dump
```
