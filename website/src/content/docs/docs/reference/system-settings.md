...
description: Reference for Northship system-wide settings and what they affect.
---
...
`Control plane hostname` serves the Northship dashboard through Caddy.
...
API Access creates scoped API keys for programmatic requests to Northship.
...
Migration settings export encrypted `.northship` bundles. The export requires a passphrase with at least 8 characters.

Bundle import is available during onboarding.

## Maintenance

Maintenance settings show disk, Docker, Northship data paths, build artifacts, database backup storage, APT cache, system logs, cleanup actions, and recent history.
...
Northship also enforces one active deployment per service.
