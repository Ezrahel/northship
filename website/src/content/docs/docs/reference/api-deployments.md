...
```bash
curl -X POST "$NORTHSHIP_URL/api/services/svc_web/deployments" \
...
```bash
curl "$NORTHSHIP_URL/api/services/svc_web/deployments" \
  -H "Authorization: Bearer $NORTHSHIP_API_KEY"
```
  ...
  "deployments": [
    {
      ...
      "status": "running",
      "trigger": "manual",
      "imageTag": "northship-svc_web-dep_123",
      "containerName": "northship-svc_web-stable",
...
```bash
curl -X POST "$NORTHSHIP_URL/api/deployments/dep_123/abort" \
  -H "Authorization: Bearer $NORTHSHIP_API_KEY"
```
...
If the deployment cannot be aborted, Northship returns `409`.
...
```bash
curl "$NORTHSHIP_URL/api/deployments/dep_123/logs" \
  -H "Authorization: Bearer $NORTHSHIP_API_KEY"
```
...
```bash
curl -N "$NORTHSHIP_URL/api/deployments/dep_123/stream" \
  -H "Authorization: Bearer $NORTHSHIP_API_KEY"
```
...
```bash
curl -N "$NORTHSHIP_URL/api/services/svc_web/runtime-logs/stream" \
  -H "Authorization: Bearer $NORTHSHIP_API_KEY"
```
