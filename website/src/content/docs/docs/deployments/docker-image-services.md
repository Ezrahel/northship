...
Docker image services skip the source build. Northship pulls the configured image and runs it directly on the server.
...
- You need full control over the image build outside Northship.
...
Northship validates the image reference before creating the service.

## Web Images

For a web image, set the internal port the container listens on. Northship starts a temporary container, probes the port, and only switches traffic after the container is reachable.

If the container starts but the port never responds, check the image logs. Northship attempts to detect port mismatch hints from container output.

## Worker Images

For a worker image, choose `worker` runtime mode. Northship starts the container without a public port and checks that the process stays running.

Worker image deployments reload Caddy only to keep routing state in sync with the rest of the project.

## Hot Swaps

For web Docker image services, Northship uses a temporary container and an active port swap:
