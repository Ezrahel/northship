...
A static site is a web service with a `Static output` directory. Northship builds the source service, creates an image, copies the static output directory from that image into `DATA_DIR/static-sites/{serviceId}`, and serves the files through Caddy.
...
If the folder does not contain `index.html`, Northship treats it as a failed static deployment. That usually means the app is server-rendered or the output directory is wrong.
...
For custom commands, Northship expects the static output path to match the output inside the built image. For auto-detected builds, use the framework's output folder.
