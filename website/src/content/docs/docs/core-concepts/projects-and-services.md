...
description: Understand how Northship groups deployable apps, databases, domains, and runtime settings.
sidebar:
  order: 1
---

Northship uses projects to group services that belong to the same application stack. A project can contain source-built apps, Docker image services, background workers, static sites, and databases.
...
Projects are also the boundary for database variable suggestions. When you add an app service, Northship can suggest variables that reference database services in the same project.

## Source Services

Source services are connected to code. Northship can read from:
...
Northship builds source services with Railpack and BuildKit, then runs them with Docker.
...
Northship can create these database engines:
...
Northship tracks service status separately from deployment status. A service can be active while a newer deployment is queued or building.
...
Northship keeps the runtime pieces visible: Docker containers, persistent database volumes, Caddy routes, BuildKit builds, generated service domains, variables, logs, backups, and maintenance all stay close to the service they affect.
