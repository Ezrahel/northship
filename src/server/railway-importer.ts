...
function classifyRailwayServiceSource(serviceName: string, sourceInfo?: RailwayServiceSourceInfo, trigger?: { repository?: string | null }): RailwayServiceClassification {
  ...
  if (sourceInfo) {
    ...
    return {
      ...
      unsupportedReason: "This Railway service is not backed by a Git repository or Docker image source Northship can import yet."
    };
  }
  ...
  return {
    ...
    unsupportedReason: "This Railway service does not expose a Git repository or Docker image source Northship can import yet."
  };
}
...
${serviceInstanceCommandSelection}
  ...
  for (const edge of servicesEdges) {
    ...
    // Database imports are recreated with fresh Northship-managed credentials.
