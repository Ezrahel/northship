...
const updateRemoteName = "northship-updates";
...
function imageManualUpdateCommand() {
  return "cd /opt/northship && sudo docker compose pull northship && sudo docker compose up -d northship";
}
...
async function runImageUpdate() {
  const currentCommit = imageCurrentCommit();
  if (!currentCommit) {
    throw new Error("This Docker image was built without NORTHSHIP_COMMIT_SHA, so Northship cannot compare updates.");
  }
  ...
  if (currentCommit === targetCommit) {
    appendLog("Northship is already on the latest published commit.");
  ...
  appendLog("Image update queued. Northship will restart when Docker replaces the container.");
  activeRun.status = "succeeded";
}
...
async function runUpdate() {
  activeRun = {
    ...
    targetCommit: null,
    restartQueued: false,
    logs: ["Checking GitHub for Northship updates..."],
    error: null
  };
  ...
  try {
    ...
    if (dirty) {
      throw new Error("Cannot update while the Northship working tree has local changes.");
    }
    ...
    if (currentCommit === targetCommit) {
      appendLog("Northship is already on the latest commit.");
      activeRun.status = "succeeded";
      return;
    }
    ...
    if (queueRestart()) {
      activeRun.restartQueued = true;
      appendLog("Restart queued. Northship will come back on the updated build.");
    } else {
      appendLog("Update built. Configure NORTHSHIP_UPDATE_RESTART_CMD to restart automatically after updates.");
    }
