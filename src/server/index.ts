...
const setupSchema = z.object({
  ...
  env: z.object({
    ...
    runtimeNetworkName: z.string().trim().min(1).default("northship-runtime"),
...
function currentRuntimeConfig() {
  return {
    ...
    runtimeNetworkName: process.env.NORTHSHIP_RUNTIME_NETWORK ?? config.runtimeNetworkName
  };
}
...
async function saveUploadedMigrationBundle(c: Context) {
  ...
  if (!isUploadedMigrationFile(bundle)) {
    throw new Error("Choose an Northship migration bundle.");
  }

  const uploadDir = mkdtempSync(join(tmpdir(), "northship-upload-"));
  const uploadPath = join(uploadDir, "bundle.northship");
...
async function applyOnboardingSettings(input: z.infer<typeof restartOnboardingSchema>, options: { generateSecretKeyIfMissing: boolean }) {
  const secretKey = input.env.secretKey || process.env.NORTHSHIP_SECRET_KEY || config.secretKey || (options.generateSecretKeyIfMissing ? generateSecretKey() : "");
  const managedEnv = {
    NORTHSHIP_SECRET_KEY: secretKey,
    ...
    NORTHSHIP_RUNTIME_NETWORK: input.env.runtimeNetworkName,
...
app.post("/api/auth/setup", async (c) => {
  if (hasAuthUsers()) {
    return jsonError("Northship has already been set up", 409);
  }
...
app.post("/api/system/dns/:provider", async (c) => {
  if (!hasSecretKey()) {
    return jsonError("NORTHSHIP_SECRET_KEY is required before saving DNS provider credentials", 409);
  }
...
app.post("/api/system/ai/providers/:provider", async (c) => {
  ...
  if (!hasSecretKey()) {
    return jsonError("NORTHSHIP_SECRET_KEY is required before saving AI provider credentials", 409);
  }
...
serve({ fetch: app.fetch, port: config.port, hostname: config.host }, (info) => {
  console.log(`Northship control plane listening on http://${info.address}:${info.port}`);
});
