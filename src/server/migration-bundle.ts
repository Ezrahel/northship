...
function currentRuntimeEnv() {
  return {
    NORTHSHIP_SECRET_KEY: process.env.NORTHSHIP_SECRET_KEY ?? config.secretKey,
    ...
    NORTHSHIP_RUNTIME_NETWORK: process.env.NORTHSHIP_RUNTIME_NETWORK ?? config.runtimeNetworkName,
...
async function writePayload(payloadDir: string) {
  ...
  const manifest: MigrationManifest = {
    ...
    source: {
      ...
      northshipVersion: process.env.npm_package_version ?? "unknown"
    },
    databaseDumps,
    files
  };
...
function importedRuntimeEnv(bundleEnv: Record<string, string>): Record<string, string> {
  ...
  return {
    ...
    NORTHSHIP_RUNTIME_NETWORK: current.NORTHSHIP_RUNTIME_NETWORK
  };
}
...
export async function createMigrationBundle(passphrase: string) {
  ...
  const workDir = await mkdtemp(join(tmpdir(), "northship-export-"));
  const payloadDir = join(workDir, "payload");
  const archivePath = join(workDir, "payload.tar.gz");
  const bundlePath = join(workDir, `northship-${new Date().toISOString().slice(0, 10)}-${nanoid(6)}.northship`);
...
export async function importMigrationBundle(bundlePath: string, passphrase: string): Promise<MigrationImportResult> {
  ...
  const workDir = await mkdtemp(join(tmpdir(), "northship-import-"));
  const archivePath = join(workDir, "payload.tar.gz");
  const payloadDir = join(workDir, "payload");
  ...
  try {
    ...
    if (manifest.format !== "northship-migration" || manifest.version !== 1) {
    ...
    config.secretKey = runtimeEnv.NORTHSHIP_SECRET_KEY ?? "";
