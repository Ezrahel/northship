...
${caddyTlsConfig(row.hostname)}  encode zstd gzip
  ...
  for (const row of databaseMappings) {
    ...
    addHostnameBlock(row.hostname, `${row.hostname} {
${caddyTlsConfig(row.hostname)}  respond "Northship ${dbType === "timescale" ? "TimescaleDB" : "Postgres"} is available on TCP ${row.hostPort}." 200
}`);
  }

  if (blocks.length === 0) {
    blocks.push(`http://127.0.0.1:65535 {
  respond "No active Northship routes." 404
}`);
  }

  return [`# Managed by Northship. Manual changes may be overwritten.`, ...blocks].join("\n\n") + "\n";
}
