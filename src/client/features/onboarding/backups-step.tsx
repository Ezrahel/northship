  ...
  return (
    ...
    >
      ...
      {hasR2Input ? (
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3 border border-zinc-800 bg-zinc-950/60 px-4 py-3">
          <p className="text-sm leading-6 text-zinc-400">
            Want to finish setup without Cloudflare R2? Clear these fields and Northship will keep backup storage on disk.
          </p>
          <button
            type="button"
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        />
        <TextField label="R2 bucket" value={form.r2Bucket} onChange={(r2Bucket) => update({ r2Bucket })} placeholder="northship-backups" />
        <TextField
          label={
            <span className="inline-flex items-center gap-2">
              R2 access key ID
        ...
        <div className="md:col-span-2">
          <ToggleField
            ...
            description="Northship will make a direct Cloudflare R2 request during setup when R2 fields are filled."
          />
        </div>
      </div>
    </OnboardingSection>
