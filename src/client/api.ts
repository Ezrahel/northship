...
async function request<T>(path: string, options?: RequestInit): Promise<T> {
  ...
  try {
    ...
    if (/failed to fetch|networkerror|load failed/i.test(message)) {
      throw new Error(`Could not reach the Northship API at ${path}. Check that your domain/proxy forwards /api requests to Northship, then try again.`);
    }
...
export const api = {
  ...
  exportMigrationBundle: async (passphrase: string) => {
    ...
    const fileName = disposition.match(/filename="([^"]+)"/)?.[1] ?? "northship-export.northship";
