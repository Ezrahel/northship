...
function handledRestartRunKey() {
  try {
    return window.sessionStorage.getItem("northship:handled-update-restart") ?? "";
  } catch {
    return "";
  }
}
...
function rememberHandledRestartRun(runKey: string) {
  try {
    window.sessionStorage.setItem("northship:handled-update-restart", runKey);
  } catch {
      ...
      setSuccess(
        run.restartQueued
          ? restartAlreadyHandled
            ? "Update applied. Northship is restarting."
            : "Update applied. Northship is restarting, then this page will refresh."
          : "Update built. Restart Northship to load server changes."
      );
    ...
    <div className="space-y-5">
      <section className="border border-zinc-800 bg-zinc-950/45 p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <h3 className="mt-2 font-hero text-2xl tracking-tight text-zinc-100">Northship release channel</h3>
            <p className="mt-2 max-w-2xl text-sm leading-relaxed text-zinc-400">
              {info?.installType === "image"
                ? "This install is running from the published Docker image. Northship compares the image commit with GitHub and can pull the next image when one is available."
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-3">
          <div className="border border-zinc-800 bg-zinc-900/50 p-3">
            ...
            <div className="mt-2 truncate font-mono text-xs text-zinc-200">{info?.repo ?? "ezrahel/northship"}</div>
          </div>
          <div className="border border-zinc-800 bg-zinc-900/50 p-3">
      ...
      {info?.dirty ? (
        <div className="border border-amber-500/35 bg-amber-950/25 px-4 py-3 text-sm leading-relaxed text-amber-200">
          The Northship checkout has local changes. Commit, deploy, or discard those changes before using the updater.
        </div>
      ) : null}
      ...
      {info?.installType === "image" ? (
        <section className="border border-zinc-800 bg-zinc-950/45 p-5">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-zinc-400">
            {!info.currentCommit
              ? "This image was built without commit metadata, so Northship cannot compare it with GitHub yet. Publish the image with NORTHSHIP_COMMIT_SHA to enable one-click updates."
              : info.canApplyUpdate
                ? "Northship will pull the latest GHCR image through a short-lived updater container, then replace the running app container."
          ...
          {!info.canApplyUpdate || info.status === "unknown" ? (
            <pre className="mt-4 overflow-x-auto border border-zinc-800 bg-black/35 px-4 py-3 font-mono text-[11px] leading-relaxed text-zinc-300">
              {info.updateCommand ?? "cd /opt/northship && sudo docker compose pull northship && sudo docker compose up -d northship"}
            </pre>
          ) : null}
        </section>
      ) : null}
      ...
      {info?.status === "current" && !updateRunning ? (
        <section className="flex min-h-[220px] items-center justify-center border border-zinc-800 bg-zinc-950/45 p-8 text-center">
          <div>
            ...
            <h3 className="mt-5 font-hero text-xl tracking-tight text-zinc-100">Northship is up to date</h3>
        ...
        <section className="border border-zinc-800 bg-zinc-950/45">
          <div className="flex flex-col gap-3 border-b border-zinc-800 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button type="button" className={shellButton("primary")} onClick={() => setConfirmingUpdate(true)} disabled={!canUpdate}>
              ...
              {updateRunning ? "Updating..." : info.installType === "image" ? "Pull latest image" : "Update Northship"}
            </button>
          </div>
      ...
      {info?.status === "diverged" ? (
        <div className="border border-rose-500/35 bg-rose-950/30 px-4 py-3 text-sm leading-relaxed text-rose-200">
          {info.installType === "image"
            ? "The running image commit is not an ancestor of GitHub main, so Northship will not update automatically. Publish a fresh image manually."
            : "This checkout has diverged from GitHub, so Northship will not update automatically. Pull or reconcile the repository manually."}
        </div>
      ) : null}
