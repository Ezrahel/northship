...
function formFromR2(r2: R2SettingsStatus): R2FormState {
  return {
    ...
    bucket: r2.bucket || "northship-backups",
...
function modeCopy(mode: R2StorageSettingsMode) {
  return mode === "account"
    ...
    : {
        ...
        formDescription: "Store R2 credentials in Northship for database backups.",
          ...
          {disconnecting ? (
            <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border border-rose-500/35 bg-rose-950/20 px-4 py-3">
              <div>
                ...
                <div className="mt-1 text-xs text-rose-200/75">Existing backup records stay in Northship; future R2 uploads will be disabled for this connection.</div>
              </div>
        ...
        <form onSubmit={saveConnection} className="space-y-5 border border-zinc-800 bg-zinc-950/45 p-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div>
              ...
              <FormInput value={form.bucket} onChange={(event) => setForm({ ...form, bucket: event.target.value })} placeholder="northship-backups" required />
            </div>
            <div>
